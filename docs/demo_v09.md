...
3. **[前端]** 报告完成后,鼠标悬停/点击报告中的 `[1]` 引用编号。
   - 旁白:每条引用对应一段原文 quote、一个来源 URL、一次抓取时间——结论逐条可追溯。
4. **[前端]** 点开"来源"列表。
   - 旁白:所有正文均来自白名单站点(docs.python.org 等);集合外结果只列为待核实,不进入证据。

## 场景 2:预算内必然停止 + 成本分账(约 1.5 分钟)
...
7. **[终端(可选)]** 演示熔断:`python -m orca.eval --only budget_total`(离线注入题,总额度打穿)
   - 旁白:额度打穿时链路不再调用任何模型,stop_reason 记为 total_budget_exhausted,报告为程序说明——预算内必然停止。

## 场景 3:评测一条命令(约 1.5 分钟)

8. **[终端]** `python -m orca.eval --only fact_freethread,fetch_fail,no_results`
   - 旁白:评测集 27 题,八类题型;离线题注入固定材料,可复现、不联网。
