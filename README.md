...
白名单来源、逐条引用可核实、预算内必然停止的 Deep Search 研究助手。
...
- **反思循环**(Phase 2):证据合并后由 reflector 决定继续检索或收尾;停止条件是**确定性的**(预算/轮次/查询重复/无新增证据在调用反思模型之前判定),LLM 只能建议、不能越过
- **两级预算**(必然停止):研究额度 = 总 LLM 额度 − writer 预留;总额度/总时限/页数/credits 各自熔断;每个任务以 `stop_reason` 如实记录停止原因(控制类与研究类两档,§3.4)
