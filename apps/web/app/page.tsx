...
import { isTerminal, type ReportDetail } from "@/lib/types";
...
export default function ResearchPage() {
  ...
  const [archiveDetail, setArchiveDetail] = useState<ReportDetail | null>(null);
  ...
  useEffect(() => {
    ...
    void fetch(`${API}/api/reports`, { signal: controller.signal })
      ...
      .then((items: RecentReport[]) => setRecentReports(items))
      .catch(() => undefined);
  ...
  const openArchive = async (id: number) => {
    setShowArchive(true);
    ...
    try {
      ...
      setArchiveDetail(await response.json());
    } catch {
      setArchiveError("暂时无法读取这份报告，请确认后端服务正在运行。");
    } finally {
  ...
  return <main className={`workbench-shell ${hasResearch ? "workbench-running" : "workbench-ready"}`}>
    <aside className="app-rail">
      ...
      <nav className="rail-nav" aria-label="主导航"><button type="button" className={`rail-link ${!showArchive ? "rail-link-active" : ""}`} onClick={() => setShowArchive(false)} aria-current={!showArchive ? "page" : undefined}>研究工作台</button><button type="button" className={`rail-link ${showArchive ? "rail-link-active" : ""}`} onClick={() => { setShowArchive(true); setArchiveError(null); }} aria-current={showArchive ? "page" : undefined}>历史报告</button></nav>
    ...
    <section className="research-stage">
      <header className="stage-header"><div><p className="eyebrow">{showArchive ? "历史记录" : "研究工作台"}</p><h1>{showArchive ? "历史报告" : heading}</h1><p className="stage-description">{showArchive ? "报告会在这里打开，阅读结束后可直接回到研究工作台。" : hasResearch ? (taskActive ? "正在搜索、阅读并整理资料。" : "查看研究过程与报告，或新建下一项研究。") : "输入问题，搜索资料，生成带引用的报告。"}</p></div>{hasResearch && !showArchive ? <div className="task-health"><StatusBadge status={view.status} />{taskActive && view.connecting ? <span>连接中，断线后自动重连</span> : null}</div> : null}</header>
      {showArchive ? <section className="inline-archive" aria-live="polite">
        {archiveDetail ? <><div className="archive-toolbar"><button type="button" className="quiet-action" onClick={() => setArchiveDetail(null)}>← 返回报告列表</button><a className="report-export" href={`${API}/api/reports/${archiveDetail.id}.md`} target="_blank" rel="noopener noreferrer">导出 Markdown</a></div><dl className="archive-summary"><div><dt>完成时间</dt><dd>{formatReportDate(archiveDetail.created_at)}</dd></div><div><dt>停止原因</dt><dd>{stopReasonLabel(archiveDetail.stop_reason)}</dd></div><div><dt>耗时</dt><dd>{Math.round(archiveDetail.duration_s)} 秒</dd></div></dl><div className="archive-paper"><ReportView markdown={archiveDetail.final_md} citationUrls={resolveCitationUrls(archiveDetail)} sources={archiveDetail.sources} title={archiveDetail.topic} /></div></> : <><div className="archive-list-heading"><h2>最近完成</h2><span>{recentReports.length} 份可打开报告</span></div>{archiveError ? <p className="notice notice-error">{archiveError}</p> : null}<div className="inline-report-list">{recentReports.length ? recentReports.map((item) => <button type="button" key={item.id} onClick={() => void openArchive(item.id)} disabled={archiveLoading !== null}><span><strong>{item.topic}</strong><small>{formatReportDate(item.created_at)} · {Math.round(item.duration_s)} 秒</small></span><em>{archiveLoading === item.id ? "正在打开…" : "查看 →"}</em></button>) : <p className="archive-empty">暂时没有可读取的报告。启动后端后会自动显示历史记录。</p>}</div></>}
      </section> : !hasResearch ? <form className="question-composer" onSubmit={(event) => { event.preventDefault(); void submit(); }}>
      ...
      {!showArchive && (hasResearch ? <FadeContent key={`run-${view.taskId}`} className="run-ledger" role="region" aria-label="研究进程"><div className="ledger-heading"><div><p className="eyebrow">研究进程</p><h2>{view.topic || "已恢复的研究任务"}</h2></div><dl className="research-counts"><div><dt>轮次</dt><dd>{view.roundNo || "—"}</dd></div><div><dt>来源</dt><dd>{view.progress.sources_read}</dd></div><div><dt>证据</dt><dd>{view.progress.evidence_count}</dd></div></dl></div>{view.stopReason ? <p className="stop-reason">停止原因：{stopReasonLabel(view.stopReason)}</p> : null}<Timeline items={view.timeline} active={taskActive} /></FadeContent> : <FadeContent className="quick-start" role="region" aria-label="快速开始"><div><p className="eyebrow">快速开始</p><h2>试试这些问题</h2></div><div className="quick-prompt-list">{quickPrompts.map((prompt) => <button type="button" key={prompt} disabled={busy} onClick={() => choosePrompt(prompt)}>{prompt}</button>)}</div>{recentReports.length > 0 ? <div className="recent-reports"><div className="section-row"><h2>最近完成</h2><button type="button" onClick={() => setShowArchive(true)}>查看全部</button></div><div className="recent-report-list">{recentReports.slice(0, 3).map((report) => <button type="button" key={report.id} onClick={() => void openArchive(report.id)}><span>{report.topic}</span><small>{formatReportDate(report.created_at)} · {Math.round(report.duration_s)} 秒</small></button>)}</div></div> : null}<dl className="research-limits"><div><dt>来源</dt><dd>白名单站点正文</dd></div><div><dt>输出</dt><dd>Markdown 报告与引用链接</dd></div><div><dt>中止</dt><dd>预算、页数或时长达到上限</dd></div></dl></FadeContent>)}
    </section>

    {hasResearch && !showArchive ? <FadeContent key={`report-${view.taskId}`} className="report-desk" id="research-report" role="region" aria-label="研究报告" delay={60}>
