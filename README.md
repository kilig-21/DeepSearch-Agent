...
- **两级预算**(超限即停):研究额度 = 总 LLM 额度 − writer 预留;每次模型调用前按剩余额度收紧 `max_tokens`,剩余不足最小可用输出即不调用、走程序降级;总额度/总时限/页数/credits 各自熔断;实耗超限会在终检中如实标记(`over_budget`),不静默。每个任务以 `stop_reason` 如实记录停止原因(控制类与研究类两档,§3.4)
