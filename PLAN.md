...
> **版本**: v1.7(2026-09-09)
> **修订记录**: v1.0 初稿 → v1.1 第一轮评审 → v1.2 第二轮评审 → v1.2.1 阅读层改本地 Defuddle → v1.3 落实第三轮评审(终止原因双路径、预算两级规则、SSE 恢复语义、Phase 1A 依赖修正、quote 严格化)→ **v1.4 第四轮验收评审打回后修复 P1-P7/F1-F4** → v1.5 第五轮复验仍打回,第二轮修复 R1-R4 后三验通过,**Phase 1B 正式闭环** → **v1.6 Phase 2 验收打回 → 修复 R1-R6 → 复验打回 → 零成本收尾 F1-F4 → 31 条分歧第三方独立终审,Phase 2 正式闭环** → **v1.7 Phase 3 闭环:1 个来源适配器 + H1 预登记对照实验(结论=检索路径可行但相关性质量不达标),MCP 裁剪为路线图**(均见 §11)
> **状态**: 计划书冻结(三轮评审通过)。**Phase 0、1A、1B、2、3 已完成**(1B 于 2026-09-06 三验闭环;2 于 2026-09-07 闭环;3 于 2026-09-09 闭环,全过程见 §11)。**当前阶段:Phase 4**(记忆沉淀,可选进阶)。1A 遗留的"22 条 support 断言人工复核"硬闸门,于 2026-09-07 以更强机制替代执行:F4 双轮标注(宽/严双口径)+ 31 条分歧第三方独立终审(quote-only)+ 守门测试复算自洽;标注 meta 诚实保留"未经人工全量复核",人工抽验随时可做
...
**DeepSearch**:输入一个问题,Agent 自动进行多轮网络搜索、网页阅读、交叉验证,最终产出一份**带证据链引用的中文研究报告**。对标 Perplexity Deep Research / Gemini Deep Research 的开源自建版。
...
- **结果(2026-09-05,已完成)**:✅ 验收四条全过——① 全部有报告题引用 ID 有效率 1.0;② 断言引用支持率 **0.938**(支持 21/部分 3/不支持 0,**宽口径 AI 初标**,报告全文+point 可见;定版口径见 Phase 2 结果行);③ 3 题记 N/A 不静默出分母(白名单约束下诚实拒答 1 题、注入抓取失败 1 题、熔断演示 1 题);④ 熔断稳定触发(研究额度 100 < planner 实测 155 tokens → budget_exhausted,程序说明收尾)。**LLM 校准**:glm-5.3 系列始终思考(`thinking.type` 不支持 disabled,1210 错误),以顶层 `reasoning_effort="low"` 压制思考(实测 0 思考 token);单调用超时校准 180s;思考 token 计入分账。**10 题基线**:断言引用支持率 0.938(宽口径初标) / 答案覆盖率 0.917 / 失败率 0/10 / 恶意注入安全 2/2 通过(单列)。136 tests。实测记录见 `apps/api/docs/probe_results.md`(T9~T11),评测产物见 `apps/api/eval/baselines/`
...
- **结果(2026-09-06,已完成)**:✅ 四场景全部 PASS,经外部评审**两轮打回 → 修复 → 三验通过**正式闭环。并发正确性收口:终态发布/恢复判定/快照字段同一锁临界区;完成路径同事务条件提交防后到覆盖;`chat_stream` 传输层显式失败(error 帧/提前 EOF 不落半截报告);引用修订受总预算约束(耗尽走确定性降级)。**213 tests**(后端 pytest)+ 前端 vitest 9 + `tsc` 通过。评审全过程见 §11 第四/五/六轮。**遗留硬闸门**:22 条 support 断言全量人工复核 → 已于 Phase 2 收尾以双轮标注+独立终审替代执行(见版本头状态行与 §11 第九轮)
...
- **结果(2026-09-07,已完成)**:✅ 经**验收打回(5 阻断)→ 修复 R1-R6 → 复验打回(2 阻断)→ 零成本收尾 F1-F4 → 31 条分歧第三方独立终审**正式闭环,全过程见 §11 第七/八/九轮。核心产出:① **预算从"事后记账"进化到"调用前约束"**——`clamp_max_tokens = min(请求值, 剩余-输入估算-余量)`,五类 LLM 调用点全部接入,剩余不足走程序降级,落库终检 over_budget 如实标记;② **失败/取消分账三路一致**(DB usage_json/终态事件/orca cost,失败行 cost_unknown 显式不折 0);③ **SSE 并发收口 R1-R4**(drain 同锁返回/resume 判定与取数同临界区/gap→snapshot 重对齐/error 帧与 EOF 显式失败);④ **评测诚实口径**:失败率分母=全部任务、标注不静默出分、meta 产物绑定(git commit/起止时间/计划哈希)、run 文件历史产物不篡改;⑤ **双轮标注暴露引用纪律缺口并终审定版**:pass1 宽口径 0.9672 → pass2 严格口径(quote-only)0.8279 → 31 条分歧第三方独立终审 **0.8238**(100.5/122,守门测试复算自洽;三类缺口:A 数字类 quote 无依据/B point 细节 quote 截断/C 否定性穷尽性超片段)。**主基线 27 行**(run_20260907_010146:26 完成+1 失败;同参数 45 行配对快照因 credits 冻结,单轮 vs 反思对比为"初步观察,不构成统计证明")。**v0.9 发布**:README 评测段与实现对齐、失败行成本未知如实声明。**282 tests**(后端 pytest)。**改善案例归因以事件轨迹为准**(conflict_ft 声称曾被轨迹证伪,实际改善案例为 timely_latest)
...
- **结果(2026-09-09,已完成)**:✅ P3-1~P3-4 闭环,**P3-5(orca-mcp)按 §4 裁剪条款降级为路线图**(应用价值不依赖 MCP)。① **P3-1 选型**:`docs.python.org/zh-cn/`,有效在线样本 N=8 严格锁定(fact_freethread/noanswer_pep/fact_gil/fact_venv/compare_logging_print/timely_maint/timely_status/noanswer_ml);② **P3-2 预登记**:`H1_PREREGISTRATION.md`(6101de9)实验前锁定候选定义/样本/主指标/判定规则;③ **P3-3 适配器**:`PythonZhDocsAdapter`(27e1457)基于版本化 `objects.inv`+`searchindex.js` 本地索引、三重 URL 白名单、0 Tavily 调用;实现中排掉两个真实 bug——`objects.inv` 解压用 `splitlines()+join` 破坏二进制流(裸 `\r` 字节,真实 165KB 索引 100% 复现,b0093d0 改字节偏移切片修复)、`searchindex.js` 解压后 4.8MB 超 `safe_fetch` 2MB 正文上限被截断(e62e4a1 索引抓取单独放宽 8MB 修复);④ **P3-4 对照实验**:四轮排障(两轮 bug + 一轮欠费 429 + 一轮组A被限流打挂)后干净对照——适配器真实命中 7/8、回落 0、索引元数据落库可复现。**结论(严格口径 quote-only)**:断言引用支持率 79.55%→76.19%(-3.36pp)、答案覆盖率 68.18%→38.64%(-29.55pp)、证据命中 49→74(+25)、平均 LLM tokens +163.6%。**核心发现=适配器检索相关性不达标**:`search()` token 子串匹配无 IDF,高频词(如"Python/版本/语句")命中大量无关页面(changelog/cmdline/C API),既浪费 tokens 又让部分题答不上来,最重时把 noanswer_pep 顶到 50k 单题预算耗尽而 execution_error。**三面定性结论**(N=8<10,不构成统计证明):正面=零 credits、白名单域名、能补上纯 Web 找不到证据的题(fact_gil 0→12、fact_venv 0→10);负面=检索相关性差致成本劣化(+163.6% 远超 120% 门槛);负面=无关证据致覆盖率下降且 noanswer_pep 预算耗尽失败。**已知遗留**:适配器检索相关性缺陷(IDF/标题加权/过滤无关页面)未修,列为后续改进项。产物见 `apps/api/eval/baselines/H1_FINAL_*(2026-09-09)`。**289 tests**。
...
### 10.5 当前基线与口径(2026-09-16 更新)

- **模型已换 DeepSeek**(`deepseek-v4-flash` 日常 / `deepseek-v4-pro` 报告,见 §2.2);预算与超时按 probe T12 重校准(§3.6、§8)。
- **当前基线**:`eval/baselines/run_20260912_195950.json`(27 题,生产默认反思模式)——失败率 0.0%(27/27 完成),有报告的 21 行引用有效率全 1.0,3 道注入题自动判定全 pass;在线 667,931 tokens / 43 credits,离线 41,022 / 12,单题最大 70,012。守门测试(`test_latest_baseline_meta_records_config_snapshot` / `test_latest_baseline_full_coverage`)以它为准。
- **标注类指标已按 DeepSeek 重跑(2026-09-16)**:答案覆盖率 **0.9113**(56.5/62);断言引用支持率(严格口径 quote-only,AI 双轮独立标注 + 第三方外部强模型终审)= **0.8777**(122/139)。产物:`eval/baselines/annotations_20260912_v1.json`(定版)+ `divergences_20260912_v1.md`(21 条分歧与裁定记录);守门测试 `test_final_adjudicated_annotation_score_deepseek` 复算定版分。
- ⚠️ **跨模型优劣不可单方向声称(口径敏感性,实测)**:在两者共有的 20 道题上,宽口径 **glm 0.9672 > DeepSeek 0.9202**,严格口径 **DeepSeek 0.8571 > glm 0.8279** —— **方向随标注口径翻转**。叠加两项不可控差异(标注者不同:glm 版为 AI(GLM)、本版为 Claude Code agent;报告平均长度差 38%,977 vs 1568 字符),本计划书**不声称"DeepSeek 优于 glm-5.3"**。正文出现的 **0.8238(100.5/122)是 glm-5.3 时代(2026-09-07)口径**(§4 Phase 2 结果、§11 第七/八/九轮评审记录、README 对应段),引用时必须同时标明时代。
- **人口不齐(跨时代对比的硬约束)**:glm 版质量分母 20 行,本版 23 行 —— 新增 `compare_http` / `conflict_typing` / `fact_docstring` 三题(该三题在 glm 轮为 N/A)。跨时代对比必须取**共同 20 题**口径,直接比全量分母是错的。
- **历史产物不篡改**:`eval/baselines/` 下 glm 时代的 run / table / 分歧清单保持原样 —— 它们是当时结论的证据。
- **换模型后代价(如实记录)**:DeepSeek 全量实耗约为 glm 的 2.9x(708,953 vs 245,113 tokens),`max_rounds` 取代 `evidence_sufficient` 成为首要停止原因(7 vs 2),即思维链开销买走了部分探索轮次。
...
## 第七~九轮:Phase 2 验收与闭环(2026-09-07)

### 第七轮:Phase 2 验收(**打回,5 阻断**)

评审(真实运行验证 + 离线替身,0 真实 API 调用)发现五项阻断:① `budget_total` 实耗 169 > 上限 160(run 文件 meta 亲证)——writer 传死 `_WRITER_MAX_TOKENS`,无剩余额度约束;② 同上,五类调用点仅部分接入预算前置;③ 空流 usage 丢失——settle 在流迭代之后,异常路径绕过;④ 非流式空报告以 completed 落库;⑤ README 声称"50 credits 评测"实含 9 行离线模拟,声明失实。另记录:执行会话曾声称"反思第二轮新增 howto 证据改善 conflict_ft",与事件轨迹相反(第二轮无新增 note)——**改善案例归因必须查事件轨迹**,实际改善案例为 timely_latest(ev_012/013/014)。

### 修复轮 R1-R6

| # | 修复 | 落实位置 |
|---|---|---|
| R1 | **预算调用前约束**:`clamp_max_tokens = min(请求值, 剩余 - prompt 估算 - 余量)`;planner/reader/reflector/writer/修订五个调用点全部接入;剩余 < 1024 不调用走程序降级;落库终检 over_budget 如实标记 | `budget.py` clamp_max_tokens、`graph.py` 五调用点 |
| R2 | **失败/取消分账三路一致**:chat_stream 已收 usage 后抛错 → settle 先于异常传播;CLI cmd_research 失败/取消路径落账("禁止丢账") | `llm.py`/`graph.py`/`cli.py`(bfd4dd0) |
| R3 | 非流式空正文显式失败,不再 completed | `graph.py` |
| R4 | README 声明与实现对齐(credits 在线 25/离线 10 分列,不再混称 50) | `README.md`(1375b48) |
| R5 | 发布物只声称已实现能力,评测段结论加限定(R5 第一批措辞修正 + 第二批对比结论限定"初步观察,不构成统计证明") | `README.md`(1375b48、a3d32c1) |
| R6 | 守门:基线快照 meta 自证(git commit/起止时间/计划哈希)+ 逐行预算核验 + 行完整性 | `test_eval.py`(cde0981) |

执行偏差(如实记录):重跑评测漏 `--compare` 耗 35 credits 得 27 行纯 reflect(月累计 88);用户叫停 45 行配对补跑(credits 敏感)→ **同参数配对快照冻结**,单轮 vs 反思对比降级为"初步观察"。

### 第八轮:复验(**仍打回,2 阻断**)与零成本收尾 F1-F4

复验确认 R1-R6 闭合,新增两阻断,均系历史数据与守门逻辑:① `compare_http` 失败行 tokens=None/usage_json={} (生成于 bfd4dd0 修复之前);② R6 守门 `row.get("tokens") or 0` 把 None 当 0 折算。**用户决策:不消耗 credits 重跑 45 行**——改走零成本收尾:

| # | 收尾 | 落实位置 |
|---|---|---|
| F1 | 失败行成本 `cost_unknown` 显式化(不再 `or 0` 折 0),且只允许出现在 failed/cancelled 行 | `runner.py`、`test_eval.py` 守门 |
| F2 | README/表格措辞如实:失败行成本未知、26 行 ≤ 上限 | `README.md`、结果表 |
| F3 | 评测产物 meta 自证绑定产物版本 | `runner.py` |
| F4 | **双轮标注**:pass1 宽口径(报告+quote+point 可见)0.9672 → pass2 严格口径(quote-only、隐藏策略标签、否定性/数字类偏严)0.8279;分歧 31 条涉 17 行单列,三类模式:A 数字类 quote 无依据 / B point 细节 quote 截断 / C 否定性穷尽性超片段 | `annotations_20260907_v1.json`、`divergences_20260907_v1.md` |

### 第九轮:31 条分歧终审与定版(**通过,Phase 2 正式闭环**)

第三方 AI agent 独立终审(标准=只认 quote 原文、point 不作证据、否定性/穷尽性从严、single/reflect 不互借):31 条中与 pass2 一致 28 条,偏离 3 条(2 更严、1 更宽,非单向从严)。司令部逐条复核:算术自洽;3 条偏离项 quote 亲读验证通过(fact_venv#4 取消激活≠删除重建;noanswer_ml#2 quote 无库名无目录、否定结论无据;conflict_offline#5 三段 quote 逐部分直接覆盖非 point 推断)。**定版轨迹:pass1 0.9672(宽口径)→ pass2 0.8279(严格)→ 终审 0.8238(100.5/122)**。终审附带结构检查:122 条断言 evidence 全命中、无重复键、无跨策略借证;7 个未标注行均无可标注报告,排除合理。裁定落档(ed5ed06)→ 执行会话回写 `annotations_20260907_v1.json`(31 条 final_mark + divergences final/final_note + meta final_score;pass1/pass2 原值保留作审计轨迹;b12089c)→ 司令部独立复算 100.5/122=0.8238 与 meta 三重自洽。守门测试 `test_final_adjudicated_annotation_score` 复算终审分防手改。**口径统一(用户确认)**:历史宽口径初标(1A 0.938)仅作参考,定版主指标一律用严格口径 quote-only 数字。**282 tests**。硬闸门("22 条 support 人工复核")以双轮标注+独立终审+守门复算替代执行,人工抽验随时可做。

---

## 第十轮:Phase 3 验收与闭环(2026-09-09)

Phase 3 按 §4 分五子任务推进(P3-1~P3-5),司令部逐子任务独立核验(亲读原始数据、不盲转执行会话报告):

- **P3-1 选型**:独立复核 run_20260907_010146 中 docs.python.org 命中统计(8/11/16 三组数字准确),裁定有效在线样本严格锁 N=8,拒绝放宽到 11 以维持 H1"中文场景"语言聚焦。
- **P3-2 预登记**:逐节核验 H1_PREREGISTRATION.md(6101de9)候选定义/N=8/判定规则与裁定一致,工作区干净无偷跑。
- **P3-3 适配器**:逐行核验 adapters.py 检索机制真实基于本地 Sphinx 索引而非包装 Tavily(红线),回落事件真实可见,config 默认关闭;亲自跑 pytest 287 passed。**核验盲区(如实记录)**:只核了"是否包装 Tavily"红线,未评估检索相关性质量,该缺陷由 P3-4 实验暴露。
- **P3-4 对照实验**:四轮排障。前两轮暴露两个真实 bug(见 Phase 3 结果行);第三轮欠费 429;第四轮组A被限流打挂(幸存者偏差假象,-10pp 收缩不可信)。第五轮干净对照得出三面定性结论。执行会话全程诚实标注(失败/N/A 未粉饰),机械判定"收缩"成立但归因由司令部补正为"检索相关性缺陷"。收尾时发现并修复一个测试回归:--only 子集产物污染"最新基线"守门(7558e56 修复,289 passed)。
- **P3-5 裁剪**:按 §4 裁剪条款(orca-mcp 应用价值不依赖 MCP)裁剪为路线图项。

**Phase 3 正式闭环,当前阶段转入 Phase 4(记忆沉淀,可选进阶)。**
...
- 项目名:DeepSearch(工作目录 `C:\Users\Lenovo\Desktop\ALLCODE\Orca`)
